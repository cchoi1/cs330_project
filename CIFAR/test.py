...
recall_levels_default = [0.95, 0.96, 0.97, 0.98, 0.99, 1.0]
...
if args.method_name.split('_')[0] == 'cifar10':
    test_data = dset.CIFAR10('/iris/u/cchoi1/exploring_ood/data/CIFAR10', train=False, transform=test_transform)
    num_classes = 10
elif args.method_name.split('_')[0] == 'cifar100':
    test_data = dset.CIFAR100('/iris/u/cchoi1/exploring_ood/data/CIFAR100', train=False, transform=test_transform)
    num_classes = 100

if args.num_id > 0:
    _, test_data = validation_split(test_data, val_share=float(args.num_id / len(test_data)))
...
def get_ood_scores(loader, in_dist=False):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if in_dist:
                ...
                incorrect_examples.append(data[wrong_indices])
                incorrect_labels.append(target[wrong_indices])
    ...
    if in_dist:
        return concat(_score).copy(), concat(_right_score).copy(), concat(_wrong_score).copy(), incorrect_examples, incorrect_labels
    else:
        return concat(_score)[:ood_num_examples].copy()


in_score, right_score, wrong_score, incorrect_examples, incorrect_labels = get_ood_scores(test_loader, in_dist=True)
...
print('\n\nError Detection')
show_performance(wrong_score, right_score, net, test_loader, recall_levels=recall_levels_default, method_name=args.method_name)

# /////////////// OOD Detection ///////////////
auroc_list, aupr_list, fpr_dict, sel_class_accs_list = [], [], {}, []


def get_and_print_results(ood_loader, num_to_avg=args.num_to_avg):
    aurocs, auprs, fprs, cons_id_accs, sel_class_accs, sel_class_covs, sel_class_aucs = [], [], [], [], [], [], []
    for _ in range(num_to_avg):
        ...
        measures = get_measures(
            ...
            in_score,
            model=net,
            id_test_dataloader=test_loader,
            recall_levels=recall_levels_default
        )
        aurocs.append(measures[0]); auprs.append(measures[1]); fprs.append(measures[2]);
        cons_id_accs.append(measures[3]); sel_class_accs.append(measures[4]); sel_class_covs.append(measures[5]);
        sel_class_aucs.append(measures[6])

    auroc = np.mean(aurocs); aupr = np.mean(auprs); cons_id_acc = np.mean(cons_id_accs)
    sel_class_acc = np.mean(sel_class_accs); sel_class_cov = np.mean(sel_class_covs); sel_class_auc = np.mean(sel_class_aucs)
    auroc_list.append(auroc); aupr_list.append(aupr); cons_id_accs.append(cons_id_acc)
    sel_class_accs.append(sel_class_acc); sel_class_covs.append(sel_class_cov); sel_class_aucs.append(sel_class_auc)
    for i, recall_level in enumerate(recall_levels_default):
        fpr_dict[recall_level] = np.mean([fprs[k][recall_level][0] for k in range(len(fprs))])

    print_measures_with_std(aurocs, auprs, fprs, cons_id_accs, sel_class_accs, sel_class_covs, sel_class_aucs, args.method_name)
...
ood_data = svhn.SVHN(root='/iris/u/cchoi1/exploring_ood/data/SVHN', split="test",
                     transform=trn.Compose([trn.Resize(32), trn.ToTensor(), trn.Normalize(mean, std)]), download=False)
if args.num_ood > 0:
    _, ood_data = validation_split(ood_data, val_share=float(args.num_ood / len(ood_data)))
ood_loader = torch.utils.data.DataLoader(ood_data, batch_size=args.test_bs, shuffle=True,
                                         num_workers=args.prefetch, pin_memory=True)
...
if args.method_name.split("_")[0] == 'cifar10':
    ood_data = dset.CIFAR100('/iris/u/cchoi1/exploring_ood/data/CIFAR100', train=False, transform=test_transform)
elif args.method_name.split("_")[0] == 'cifar100':
    ood_data = dset.CIFAR10('/iris/u/cchoi1/exploring_ood/data/CIFAR10', train=False, transform=test_transform)
else:
    raise ValueError
...
print('\n\nCIFAR-100 Detection') if args.method_name.split('_')[0] == 'cifar10' else print('\n\nCIFAR-10 Detection')
get_and_print_results(ood_loader)

# /////////////// Mean Results ///////////////

print('\n\nMean Test Results')
print_measures(np.mean(auroc_list), np.mean(aupr_list), fpr_dict, method_name=args.method_name)
...
if 'cifar10_' in args.method_name:
    ood_data = dset.CIFAR100('/iris/u/cchoi1/exploring_ood/Data/CIFAR100', train=False, transform=trn.ToTensor())
else:
    ood_data = dset.CIFAR10('/iris/u/cchoi1/exploring_ood/Data/CIFAR10', train=False, transform=trn.ToTensor())
